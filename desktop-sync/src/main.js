...
ipcMain.handle('tj-login', async (_event, credentials) => {
  try {
    ...
    return { ok: true };
  } catch (error) { console.error('TJ Organization login/sync failed:', error); return { ok: false, error: error instanceof Error ? error.message : 'Could not start sync.' }; }
});
